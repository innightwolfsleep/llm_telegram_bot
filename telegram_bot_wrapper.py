...
class TelegramBotWrapper:
    ...
    def load_config_file(self, config_file_path: str):
        if os.path.exists(config_file_path):
            with open(config_file_path, "r") as config_file_path:
                ...
                self.generator_params_file_path = config.get("generator_params_file_path",
                                                             self.generator_params_file_path)
    ...
    def generate_answer(self, user_in, chat_id) -> Tuple[str, str]:
        ...
        try:
            ...
            context_len = generator_script.tokens_count(context)
            ...
            for s in reversed(conversation):
                s = "\n" + s if len(s) > 0 else s
                s_len = generator_script.tokens_count(s)
    ...
    def show_options_button(self, upd: Update, context: CallbackContext):
        ...
        greeting_tokens = -1
        try:
            history_tokens = generator_script.tokens_count("\n".join(user.history))
